...
function Header({ placeholder }) {
  ...
  return (
    <header className="sticky top-0 z-50 grid grid-cols-3 bg-white shadow-md px-5 py-5 md:px-10">
      <div onClick={(e) => router.push('/')} className="relative flex items-center h-10 cursor-pointer">
      ...
      <div className="flex items-center md:border-2 rounded-full py-2 md:shadow-sm">
        ...
        />
        <SearchIcon className="text-white h-8 bg-red-400 rounded-full p-2 cursor-pointer hidden md:inline-flex md:mx-2" />
      </div>
      ...
      {search && (
        <div className="flex flex-col col-span-3 mx-auto">
          ...
          <div className="flex items-center border-b mb-4">
            <h2 className="text-2xl pl-2 flex-grow border-semibold">Number of Guests</h2>
            <UsersIcon className="h-5" />
            <input type="number" className="w-12 pl-2 text-lg outline-none text-red-400" min={1} max={10} value={guests} onChange={(e) => setGuests(e.target.value)} />
          </div>
          <div className="flex">
